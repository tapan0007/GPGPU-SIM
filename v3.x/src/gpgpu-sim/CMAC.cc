...
#define TILINGS_PER_GROUP 8
//#define TILINGS_PER_GROUP 16
...
void CMAC::updateWeights(double delta, double alpha){
  ...
  for(int i = 0; i < numNonzeroTraces; i++){
    ...
    if(f > RL_MEMORY_SIZE || f < 0){
      cerr << "f is too big or too small!!" << f << "\n";
    }

	double incrWt =tmp * traces[f];
    weights[f] += incrWt;
	if (gMaxWeightsIndex < f)
		gMaxWeightsIndex = f;
  }
}
...
void CMAC::decayTraces(double decayRate){
  ...
  for(int loc = numNonzeroTraces - 1; loc >= 0; loc--){
    ...
    f = nonzeroTraces[loc];
    if(f > RL_MEMORY_SIZE || f < 0){
      cerr << "DecayTraces: f out of range " << f << "\n";
    }
...
void CMAC::clearTrace(int f){
  ...
  if(f > RL_MEMORY_SIZE || f < 0){
    cerr << "ClearTrace: f out of range " << f << "\n";
  }
...
void CMAC::clearExistentTrace(int f, int loc){
  ...
  if(f > RL_MEMORY_SIZE || f < 0){
    cerr << "ClearExistentTrace: f out of range " << f << "\n";
  }
...
void CMAC::setTrace(int f, double newTraceValue){
  ...
  if(f > RL_MEMORY_SIZE || f < 0){
    cerr << "SetTraces: f out of range " << f << "\n";
  }
...
void CMAC::print(){
  ...
  for (int i = 0; i <= gMaxWeightsIndex; i++){
    //printf("%f,%f\n", weights[i], traces[i]);
	//if (weights[i] > 0)
	if (weights[i] != 0)
    	printf("%u: %f\n", i, weights[i]);
...
void CMAC::loadTiles(){
  ...
  if(numTilings > RL_MAX_NUM_TILINGS){
    cerr << "TOO MANY TILINGS! " << numTilings << "\n";
  }
}
...
double CMAC::computeQ(int action){
  ...
  for(int j = 0; j < numTilings; j++){
	...
	int idx = tiles[action][j];
    q += weights[idx];
	if (gMaxWeightsIndex < idx)
		gMaxWeightsIndex = idx;
  }
